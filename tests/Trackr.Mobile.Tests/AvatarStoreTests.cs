...
public sealed class AvatarStoreTests
{
    ...
    {
        var (store, api, _) = Build(avatarUpdatedUtc: null);
    ...
    {
        var (store, api, session) = Build(Marker);
    ...
    {
        var (store, api, session) = Build(avatarUpdatedUtc: null);
    ...
    /// </summary>
    private static (AvatarStore Store, ITrackrApiClient Api, AuthSession Session) Build(
        DateTimeOffset? avatarUpdatedUtc)
    {
        ...
        var store = new AvatarStore(api, session);
